...
function Question() {
  ...
  const handleReset = () => {
    ...
    setCorrectAnswers(0);
    setIsHighlighted(false); // Reset highlight state
  };
    ...
    <div>
      {currentQuestionIndex < questions.length ? (
        <div>
          ...
          <div id='prim-div'>
            <div id='sec-div'>
              <p id='question' style={{ color: isHighlighted ? 'red' : 'inherit' }}>
                {questions[currentQuestionIndex].text}
              </p>
              <ul>
                {questions[currentQuestionIndex].options.map((option) => (
                  <li key={option.id}>
                    <button onClick={() => handleOptionClick(option.isCorrect)}>
                      {option.text}
                    </button>
                  </li>
                ))}
              ...
              <div>
                <button className='high' onClick={handleHighlight}>Highlight</button>
                <button className='high' onClick={handleUnhighlight}>Unhighlight</button>
              </div>
            </div>
          </div>
        </div>
